...
$('#login').click(function(){
  $.ajax({
	...
	$('#sign_in_information').animate({opacity:0},600);
	$('#member').css({"display":"none"});
	$('#member_after').css({"display":"inline"});
      }
      else{
	$('#sign_in_form').html("<div id=\"login_success\">"+data+"</div><script type=\"text/css\">#login_sucess{background:#111111;}</script>");
	$('#sign_in_form').animate({opacity:0}, 600, function(){
	  $('#sign_in_form').html("<ul class=\"menubar\"><li><a id=\"create-user\" href=\"javascript:void(0);\"><span id=\"create_user\">註冊</span></a></li><li><a href=\"#\"><span class=\"login\">登入</span></a></li></ul>");
	}
	);
...
$(function() {
  $( "#open_act_page" ).dialog({
    ...
    buttons: {
      "Create": function() {
	alert($('textarea[name=introduction]').val());
	$.ajax({
	  data:{
...
$('#item').change(function(){
  var a = '['+$('#item option:selected').text()+'] ';
...
$.get('get_act.php',function(data){
  ...
  while(i<hot.length){
    ...
    but = $("<button " + " type='button'" + " id=" + "orange_button"
      +" class=" + "orange"  +" name=" + hot[i].a_id
      + " >" + "我要參加" + "</button>");
    ...
    $("#content").append("<div " + "id= " + "box_5 " + ">"
      ...
      +"</p>"
      +"</div>"+ "<div>"
      +"<button " + " type='button'" + "href=" + "#" + " id="+"test"+i
      +" class='orange'" +  "name=" + hot[i].a_id
      + " >" + "我要參加" + "</button>"
      +"</div>" + "</div>" + "</div>");
